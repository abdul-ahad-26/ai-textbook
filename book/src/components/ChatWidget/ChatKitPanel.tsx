...
export default function ChatKitPanel() {
  ...
  // Subscribing to auth keeps this component re-rendering on sign in/out; identity
  // itself is proven to the backend by the bearer token, not by these values.
  useAuth();
  ...
  const injectingFetch: typeof fetch = (input, init = {}) => {
    ...
    // Send the Better-Auth session token; the backend verifies it and derives the
    // user + profile server-side (the old X-User-* headers were spoofable).
